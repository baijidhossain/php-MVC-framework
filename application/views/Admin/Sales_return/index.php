...
<div class="wrapper">
        ...
        <div class="col-md-12">
          <div class="box box-widget">
            <div class="box-header with-border ">
              ...
              <div class="box-tools">
                <a href="<?= APP_URL ?>/admin/sales_return/add" class="btn btn btn-sm  bg-color">
                    ...
                    if (empty($data['PurchaseReturns']['paginateData'])) {
                      ...
                      foreach ($data['PurchaseReturns']['paginateData'] as  $purchase) { ?>
                        ...
                        <tr class="item_row">
                          ...
                          <td><?= date_create($purchase['sales_date'])->format('d M, Y '); ?></td>
                          ...
                          <td class="text-center">
                            <div class="btn-group btn-group-sm">
                              ...
                              <ul class="dropdown-menu dropdown-menu-right border" role="menu">
                                <li><a href="<?= APP_URL ?>/admin/sales_return/views/<?= $purchase['id'] ?>"> <i class="fa fa-eye text-purple"></i> View Sales</a></li>
                                <li><a href="<?= APP_URL ?>/admin/sales_return/return_edit/<?= $purchase['id'] ?? "" ?>"><i class="fa fa-edit text-purple"></i> Edit</a></li>
                                <li><a data-toggle="modal" data-target="#myModal" href="<?= APP_URL ?>/admin/sales_return/payment_view/<?= $purchase['id'] ?>"><i class="fa  fa-money text-purple"></i> View Payments</a></li>
                                <li><a data-toggle="modal" data-target="#myModal" href="<?= APP_URL ?>/admin/sales_return/paynow/<?= $purchase['id'] ?>"><i class="fa   fa-hourglass-half text-purple"></i> Pay Now</a></li>

                                <li><a href="<?= APP_URL ?>/admin/sales_return/printinvoice/<?= $purchase['id'] ?? "" ?>"><i class="fa  fa-print text-purple"></i> Print</a></li>
                                <li><a href="<?= APP_URL ?>/admin/sales_return/pdfSalesReturnInvoice/<?= $purchase['id'] ?>"><i class="fa  fa-file-pdf-o text-purple"></i> PDF</a></li>
  ...
  <script>
    ...
    function paymentDelete(id) {
      ...
      if (conf) {
        window.location = "<?= APP_URL ?>/admin/sales_return/delete_payment/" + id;
      }
    }
    ...
    function invoiceDelete(id) {
      ...
      if (conf) {
        window.location = "<?= APP_URL ?>/admin/sales_return/delete_sales/" + id;
      }
    }
    ...
    $(".btn").on('click', function() {
      ...
      if ($('.item_row').length > 0) {
        $('.item_row').removeClass('item_row_height')
      }

      $(this).parent('div').parent('td').parent('tr').addClass('item_row_height');
