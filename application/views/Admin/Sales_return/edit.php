...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <form action="<?= APP_URL ?>/admin/sales_return/update/<?= $data['salesReturn']['id'] ?? "" ?>" method="post" id="form" enctype="multipart/form-data">
        <div style="margin-top: 20px;" class="row">

          <div class="col-md-12">
            <div class="box box-widget">
              <div class="box-header with-border ">
                <h3 class="box-title">Sales Return Info</h3>
              </div>
                ...
                <div class="row">
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      <label for="supplier">Customer <span class="text-danger">*</span></label>
                      <select class="form-control select2" name="customer">
                        <option value="">Select customer</option>
                        <?php
                        foreach ($data['customers'] as  $customer) { ?>
                          <option <?= $data['salesReturn']['customer'] == $customer['id'] ? "selected" : "" ?> value="<?= $customer['id'] ?>"><?= $customer['name'] ?></option>
                        <?php } ?>
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      <label for="date">Sales Return Date <span class="text-danger">*</span></label>
                      <input type="date" class="form-control" name="sale_date" value="<?= date_create($data['salesReturn']['sales_date'])->format('Y-m-d') ?>">
                    </div>
                  </div>
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <select class="form-control select2" name="status">
                        ...
                        <option <?= $data['salesReturn']['status'] == 'Return' ? 'selected' : '' ?> value="Return">Return</option>
                        <option <?= $data['salesReturn']['status'] == 'Cancel' ? 'selected' : '' ?> value="Cancel">Cancel</option>
                  ...
                  <div class=" col-md-4">
                    <div class="form-group">
                      ...
                      <textarea class="form-control" name="note" rows="1" style="resize: vertical;"><?= $data['salesReturn']['note'] ?? "" ?></textarea>
                    </div>
                  </div>
              ...
              <div class="box-body">
                <div class="row">
                  ...
                  <div class="col-md-12 text-right">
                    <button type="submit" class="btn btn-sm bg-color save float-right" style="width:100px;">Update</button>
                  </div>
                </div>
                        ...
                        <tbody>
                          ...
                          if (empty($data['paymentinfo'])) {
                            ...
                            foreach ($data['paymentinfo'] as $key => $payinfo) { ?>
                              ...
                              <tr>
                                <td><?= $i ?></td>
                                <td><?= date_create($payinfo['created'])->format('d M, Y') ?></td>

                                <td><?= $payinfo['user'] ?></td>
                                <td><?= $payinfo['payment_type'] == 1 ? 'Cash' : 'Bank' ?></td>
                                <td><?= $payinfo['payment_note'] ?></td>
                                <td><?= $payinfo['payment_amount'] ?></td>
                                <td><a href="javascript:paymentDelete(<?= $payinfo['id'] ?>)"><i class="fa fa-trash text-danger"></i> </a></td>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('.product').change(function() {
        ...
        var url = "/admin/purchase_return/Invoice/add";
        $.ajax({
          url: url,
          type: "POST",
          data: {
    ...
    function invoiceUpdate() {
      ...
      var discount_on_all = $('.discount_on_all').val();
      var url = "/admin/purchase_return/Invoice/invoiceUpdate";
      $.ajax({
        url: url,
        type: "POST",
        data: {
  ...
  <script>
    ...
    inputfile.addEventListener('change', function() {
      ...
      if (previewImage !== undefined) {
        ...
        file_display(previewImage);
      }

    });

    //   Start file display function
    function file_display(previewImage) {
